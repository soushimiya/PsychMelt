...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Array<haxe.extern.EitherType<String, Float>>> = [
		...
		['Sick!',      1.0],   // From 90% to 99%
		['Perfect!!',  1.0]    // The value on this one isn't used actually, since Perfect is always "1"
	];
	...
	override public function create()
	{
		...
		if(ClientPrefs.data.downScroll)
			botplayTxt.y = timeBar.y - 156;
		else
			botplayTxt.y = timeBar.y + 133;
	...
	{
		var epic:Int = ratingsData[0].hits;
	...
	function startSong():Void
	{
		...
		if(autoUpdateRPC) DiscordClient.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")", iconP2.char, true, songLength);
		#end
	}
	...
	{
		#if DISCORD_ALLOWED
		if (health > 0 && !paused && autoUpdateRPC) DiscordClient.changePresence(detailsPausedText, SONG.song + " (" + storyDifficultyText + ")", iconP2.char);
		#end
	...
	{
		final hpBound = (healthBar.bounds != null ? healthBar.bounds.min : 0); // custom health bounds go brrrr
		if (((skipHealthCheck && instakillOnMiss) || health <= hpBound) && !practiceMode && !isDead)
		{
			...
			if(autoUpdateRPC) DiscordClient.changePresence("Game Over - " + detailsText, SONG.song + " (" + storyDifficultyText + ")", iconP2.char);
			#end
			return isDead = true;
		}
		return false;
	...
	public function triggerEvent(eventName:String, value1:String, value2:String, ?strumTime:Float) {
		...
		switch(eventName) {
			...
			case 'Change Scroll Speed':
				if (songSpeedType != "constant")
				{
					...
					if(flValue2 <= 0)
						...
						songSpeedTween = FlxTween.num(songSpeed, newValue, flValue2 / playbackRate, {ease: FlxEase.linear, onComplete:
							function (_)
							{
								songSpeedTween = null;
							}
			...
			case 'Set Property':
				//do better things later
				trace("Set Property not supported dumbass");
	...
	function moveCameraSection(?sec:Null<Int>):Void {
		...
		final isDad:Bool = (!SONG.notes[sec].mustHitSection);
		moveCamera(isDad);
		camX = camFollow.x;
		camY = camFollow.y;
	}
	...
	private function popUpScore(?note:Note):Void
	{
		...
		if (ClientPrefs.data.hideHud || (!showRating && !showCombo && !showComboNum))
			return; // fuck it, skip shit
		...
		final mult = (PlayState.isPixelStage ? daPixelZoom * 0.85 : 0.7);
		...
		if (showComboNum)
		{
			final seperatedScore:Array<Int> = [
				for (i in 0...(combo >= 1000 ? 4 : 3))
					Math.floor(combo / Math.pow(10, i)) % 10
			];
			seperatedScore.reverse();

			var daLoop:Int = 0;
			final numMult = (PlayState.isPixelStage ? daPixelZoom : 0.5);
		...
		if (showCombo)
		{
			if (!showComboNum) // simulate generated combo nums
			{
				final fakeNumX = placement + (43 * 2) - 90 + ClientPrefs.data.comboOffset[2];
				if (fakeNumX > xThing) xThing = fakeNumX;
			}
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		if (!SONG.notes[curSection].mustHitSection) {
			switch(note.noteData % 4) {
				case 0:
					camFollow.x = camX - camStrength;
					camFollow.y = camY;
				case 1:
					camFollow.y = camY + camStrength;
					camFollow.x = camX;
				case 2:
					camFollow.y = camY - camStrength;
					camFollow.x = camX;
	...
	public function goodNoteHit(note:Note):Void
	{
		...
		if (!note.isSustainNote)
		{
			...
			health += note.hitHealth * healthGain - 0.005;
		}
